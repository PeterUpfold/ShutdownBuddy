...
try {
    New-Item -ItemType Directory $sdbPath
}
catch {
...
if ($DebugLog) {
    Set-ItemProperty -Path $regRoot -Name "DebugLog" -Value 1
}
else {
    Set-ItemProperty -Path $regRoot -Name "DebugLog" -Value 0
}

Set-ItemProperty -Path $regRoot -Name "EvaluationIntervalSeconds" -Value $EvaluationIntervalSeconds
Set-ItemProperty -Path $regRoot -Name "ShutdownAfterIdleForSeconds" -Value $ShutdownAfterIdleForSeconds
